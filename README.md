...
| [unipdf#524](https://github.com/unidoc/unipdf/issues/524) | TJ kerning array silently drops word-spacing | `interpretTJArray` (`content.go`): numeric elements ≤ −120 thousandths now emit a synthetic space before the next string segment; applies to `Content()` / `GetStyledTexts()` path. Threshold 120 is a conservative word-gap value. | Fork fix — `Content()`/`GetStyledTexts()` path |
