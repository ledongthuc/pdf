...
func Open(file string) (*os.File, *Reader, error) {
	...
	// 使用内存数据构造一个新的 Reader
	tempReader := bytes.NewReader(allData)

	// 这里把原来 `NewReader(f, fi.Size())` 改成传入新的 tempReader，以及长度改成 allData 的长度
	reader, err := NewReader(tempReader, int64(len(allData)))
